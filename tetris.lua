...
W=io.write W"\027[?25l\027[2J"F=math.floor S="  \027[m"M=math.fmod f={}t=0 z=0 u=1 c=0
...
repeat
	while o==N do
		...
		a={}
		for i=0,15 do a[F(i/4)*12+M(i,4)+5]=o and o[i+1]end
	end
	if c then
		for i=1,16 do
			W(M(i,4)==1 and"\027["..2+F(i/4)..";27H"or"",1<=M((p[i]or 0)/g,2)and"\027[44m"or"",S)
		end
		W("\027[7;27HLevel: ",u,"\027[9;27HLines: ",z,"\027[;H")
		for i=25,276 do
			W((f[i]or M((a[i]or 0)/r,2)>=1)and"\027["..44+(f[i]or 0).."m"or"",S)q=1>M(i,12)and W"\n"
		end
	else E"sleep .009"t=t+1 end
	e=c=="q"h=0 s=r b=a
	if c==" "then E"sleep .005"else c=io.read(1)end
	r=c=="x"and M(r*2,15)or(c=="y"or"A"==c)and(r<2 and 8 or r/2)or r
	if c=="B"or c==" "or t>53-u*5 then t=0 h=12 c=c or 0 end
	l=a a={}
	for i,q in pairs(l)do a[h+i+(c=="C"and 1 or c=="D"and-1 or 0)]=q end
	for i=1,276 do
		if f[i]and 1<=M((a[i]or 0)/r,2)then
			r,a=s,b
			if h>0 then
				E"sleep .2"
				repeat c=io.read(1)until c==N
				for i,q in pairs(a)do
					c=i<25 and"q"or c
					f[i]=f[i]or 1<=M(q/r,2)and 0
				end
				o=N
				if c==N then
					for i=24,252,12 do
